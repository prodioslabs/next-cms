...
function getValidationSchemaForField(field) {
  ...
  switch (field.type) {
    ...
    case "rich-text":
    case "slug": {
      schemaBasedOnType = z2.string().min(1);
      break;
    }
    case "date": {
      schemaBasedOnType = z2.string().datetime();
      break;
    }
    case "number": {
      schemaBasedOnType = z2.number();
      break;
    }
    case "image": {
      schemaBasedOnType = z2.object({
        url: z2.string().min(1),
        width: z2.number().int(),
    ...
    case "icon": {
      schemaBasedOnType = z2.object({
        ...
        iconLib: z2.enum(["lucide"])
      });
      break;
    }
    case "color": {
      schemaBasedOnType = z2.string().startsWith("#");
      break;
    }
    case "select": {
      schemaBasedOnType = z2.object({
...
function getValidationSchemaForSchema(schema) {
  let validationSchema = z2.object({});
  Object.entries(schema).forEach(([fieldKey, field]) => {
